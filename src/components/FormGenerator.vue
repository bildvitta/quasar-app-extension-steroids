<template>
  <div>
    <div :class="classes">
      <div v-for="(field, key) in groupedFields.visible" :key="key" :class="getFieldClass(key)">
        <slot :name="`field-${field.name}`" :field="field">
          <field :value="value[field.name]" :error="errors[key]" :field="field" @input="emitValue(field.name, $event)" />
        </slot>
      </div>
    </div>
    ...
    <div v-for="(field, key) in groupedFields.hidden" :key="key">
      <slot :name="`field-${field.name}`" :field="field">
        <field :value="value[field.name]" :field="field" @input="emitValue(field.name, $event)" />
      </slot>
    </div>
  </div>
</template>
